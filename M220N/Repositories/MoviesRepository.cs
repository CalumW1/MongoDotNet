...
namespace M220N.Repositories
{
    public class MoviesRepository
    {
        ...
        public async Task<Movie> GetMovieAsync(string movieId, CancellationToken cancellationToken = default)
        {
            try
            {
                return await _moviesCollection.Aggregate()
                    ...
                    .Lookup(_commentsCollection, m => m.Id, 
                    c => c.MovieId, (Movie m) => m.Comments)
                    // Ticket: Get Comments
            ...
            )
        {
            ...
            var sortTitle = Builders<Movie>.Sort.Descending(m => m.Title);
            // create project to only return the id and title fields 
            var ProjectTitle = Builders<Movie>.Projection.Include(m => m.Id).Include(m => m.Title);
            return await _moviesCollection.Find<Movie>(countryfilter).Sort(sortTitle).Project<MovieByCountryProjection>(ProjectTitle).ToListAsync(cancellationToken);
        }
