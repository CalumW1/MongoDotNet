        ...
        */
            ...
            CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                // create a new user with name, email and hashed password (Hashed with helper function)
                user.Name = name;
                user.Email = email;
                user.HashedPassword = PasswordHashOMatic.Hash(password);

                InsertOneOptions insertOne = new InsertOneOptions();

                // insert a new user into the user collection;
                await _usersCollection.WithWriteConcern(WriteConcern.WMajority).InsertOneAsync(user, insertOne, cancellationToken);
        ...
        public async Task<UserResponse> LoginUserAsync(User user, CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                // check if the userID is equal, to the email address within this fuction. 
                var sessionsFilter = Builders<Session>.Filter.Eq(m => m.UserId, user.Email);

                var sessions = await _sessionsCollection.Find<Session>(sessionsFilter).FirstOrDefaultAsync();

                // if session collection is null create new session
                if (sessions == null)
                {
                    await _sessionsCollection.UpdateOneAsync(
                ...
                // else if session exists update email and auth token.
                else
                {
                    sessions.Jwt = user.AuthToken;
                    sessions.UserId = user.Email;
                }
