    ...
    public class CommentsRepository
    {
            ...
            CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                await _commentsCollection.InsertOneAsync(newComment, insertOne, cancellationToken);
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            // create a filter for getting the email address from the comments
            //var commentFilter = Builders<Comment>.Filter.Eq(c => c.Email, user.Email);
            //var findComments = await _commentsCollection.Find<Comment>(commentFilter).FirstOrDefaultAsync();

            
            // check if the user email address is the same as the comment email. 
            return await _commentsCollection.UpdateOneAsync(
                    Builders<Comment>.Filter.Where(c => c.Email == user.Email),
                    Builders<Comment>.Update.Set("text", comment).Set("movie_id", movieId),
